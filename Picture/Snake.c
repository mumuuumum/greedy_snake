...
#define MAXSNAKELEN 40               //蛇最大的长度
#define HEAD '@'                     //蛇头
#define BODY '*'                     //蛇身
#define FOOD '$'                     //食物
#define PROP '#'                     //穿墙一次的特殊食物
#define EASYLIM 70                   //简单模式的极限速度
#define MEDIUMLIM 65                 //中等模式的极限速度
#define DIFFICULTLIM 60              //困难模式的极限速度
//
...
//
void Details ();                                                              //游戏详细规则（实现进入规则界面和退出）
void Speeder ();                                                              //三轮跑得很快
...
int main () {
    ...
    memset(LeSnake,0, sizeof(int) * MAXSNAKELEN);
    memset(VeSnake,0, sizeof(int) * MAXSNAKELEN);
    LeSnake [0] = (WIDTH - 1) / 3;               //蛇头与蛇身的横坐标
    VeSnake [0] = (HIGHT - 1) / 3;               //蛇头与蛇身的纵坐标
    lswsad = '0';                                                            //记录用户上次敲击的键
    foodneeded = 1;                                                           //1表示被需要
    propneeded = -(rand() % 6);
    score = 0;                                                                //得分
