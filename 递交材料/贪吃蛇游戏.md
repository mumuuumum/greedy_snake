...
制作人：张淞钦                                                                                                                                                                    学号：211250147

平台（操作系统）：Windows

编译选项：gcc -o

使用的第三方库：windows.h;conio.h

思路：从状态i至状态i+1：第k节蛇身的状态i+1下的位置是第k-1节蛇身的状态i下的位置

个人认为写的比较好的地方：实现了个人2021年内最高的函数封装程度，main函数中的思路清晰；

​                                                  函数的声明排列顺序是按照进入游戏过程的顺序，方便定位debug；

​                                                  实现了和用户较多的交互；

​                                                  实现了穿墙模式和非穿墙模式的融合；

​                                                  实现了三种游戏难度，不同难度在特殊道具、极限速度、加速度上不同。

一些特殊要求：编译成功后，用运行exe文件出现混乱，请调整窗口大小直至获得游戏效果；
...
|          | 特殊道具出现间隔 | 休眠下界（极限速度） | 趋于休眠下界的比率（加速度） | 达到极限速度所需获取食物数量 |
| -------- | ---------------- | -------------------- | ---------------------------- | ---------------------------- |
| 简单模式 | 1~6              | 70                   | 0.98                         | 约31                         |
| 中等模式 | 1~12             | 62                   | 0.97                         | 约20                         |
| 困难模式 | 1~15             | 50                   | 0.95                         | 13                           |
